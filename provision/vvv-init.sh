...
mysql -u root --password=root -e "GRANT ALL PRIVILEGES ON ${DB_NAME}.* TO root@localhost IDENTIFIED BY 'root';"
echo -e "\n DB operations done.\n\n"
...
if [[ ! -f "${VVV_PATH_TO_SITE}/public_html/wp-config.php" ]]; then
  ...
  noroot wp core config --dbname="${DB_NAME}" --dbuser=root --dbpass=root --dbprefix=tv_ --quiet --extra-php <<PHP
...
if ! $(noroot wp core is-installed); then
  ...
  noroot wp core ${INSTALL_COMMAND} --url="${DOMAIN}" --quiet --title="${SITE_TITLE}" --admin_name=admin --admin_email="abazeed@msn.com" --admin_password="admin"
else
